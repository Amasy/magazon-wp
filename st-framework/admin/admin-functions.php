    ...
    */
    function upload(){
        echo  '<div class="STpanel-input-box box-upload">';
         ...
         <input class="bp-upload-button button-secondary" for-id="'.$id.'" data-type="'.$input_type.'" type="button" value="'.__('Select Image','magazon').'" />';
         $input.='<div style="clear:both"></div>';
            ...
            // $input .= (($image!='') ? ''.__('View full image','magazon').'</a>' : '');
            $input .='</div>';
        
          
          $input.='<a href="#" class="remove_image button-secondary">'.__('Remove','magazon').'</a>';
    ...
    function select(){
        ...
        if(isset($this->field['multiple']) && $this->field['multiple']){
            ...
            $input .='<option value="">'.__('Select','magazon').'</option>';
    ...
    function  radio_checked($value=''){
        ...
        $this->class='';
        if($this->is_default){
            $cvalue = isset($this->field['default']) ? $this->field['default'] : ''; 
        }else{
           $cvalue = isset($this->values[$this->field['name']]) ? $this->values[$this->field['name']] : '';     
        }
    ...
    function layout(){
         ...
         foreach($this->field['options'] as $k => $image){
             ...
             $input.='<label class="layout-label '.(($class!='') ? 'layout-label-'.$class : '' ).'"><input value="'.htmlspecialchars($k).'" class="STpanel-radio-input '.$class.'" type="radio" '.$check.' name="'.$this->name().'" /><img src="'.$image.'" alt =""/></label>';
             $input.='</div>';
         }
    ...
    function color(){
         ...
         $input = '<div class="colorSelector-wrap"> <div class="colorSelector"><div style="background-color: #'.$this->value().'"></div></div>';
        
         $input .= '<label><input class="colorSelector-input" name="'.$this->name().'" maxlength="6" size="6" value="'.$this->value().'" /></label>';
    ...
    function  style(){
         ...
         <div class="line-size js-slider-wrap">
             <p>'.__('Letter spacing','magazon').': <span class="amount">'.$values['letter-spacing'].'</span>
             <input type="hidden" value="'.$values['letter-spacing'].'" class="letter-spacing hidden-amount"  name="'.$name.'[letter-spacing]" />
             <span class="unit">
             <select class="letter-spacing-unit style-unit" name="'.$name.'[letter-spacing-unit]" >';
             
             foreach($this->units as $u){
                
                 $slected ='';
                if($values['letter-spacing-unit']==$u){
                    $slected =' selected ="selected" ';
                }
                
                $input.='<option value="'.$u.'" '.$slected.'>'.$u.'</option>';
             }
             ...
             <div class="js-s-g  js-slider-percent"></div>
             <div class="js-s-g  js-slider-em"></div>
             <div class="js-s-g  js-slider-pt"></div>
         </div>';
         */
        ...
        // for google font
         $input .='<label>'.__('Font family','magazon').'</label>
         <select class="font-family chzn-select" name="'.$name.'[font-family]">';
         foreach($this->fonts as $font_name => $font_url){
            
             $slected ='';
                if($values['font-family']==$font_name){
                    $slected =' selected ="selected" ';
                }
             
            $input.='<option '.$slected.' url="'.esc_attr($font_url).'" value="'.esc_attr($font_name).'">'.esc_html($font_name).'</option>';
         }
         $input.='</select>';
         
         // for font style 
         $input .='<label>'.__('Font style','magazon').'</label>';
         ...
         $input.='</select>';
         // for font Wieght 
          $input .='<label>'.__('Font weight','magazon').'</label>';
         ...
         $input.='<h3 class="smtitle">'.__('Font Preview','magazon').'</h3>';
    ...
    function usage(){
         global $post;
          ...
          ?>
          <div class="usage">
             <div><?php _e('PHP get data','magazon'); ?>: <code><?php echo htmlspecialchars($php_code); ?></code></div>
          </div>
          <?php
    }
    */
            ...
            <ul class="sortable st-ui-list">
                ...
                <li>
                        ...
                        <div title="Click to toggle" class="ui-handlediv"><br></div>
                         <a href="#" class="remove stwrmt button-secondary"><?php _e('Remove','magazon'); ?></a>
                        <h3 class="st-hndle "><?php _e('Title:','magazon'); ?> <span><?php echo esc_html($v['title']); ?></span>
                    	...
                    	<div class="inside">
                        	...
                        	<div class="widget-content">
                                 <?php // echo var_dump($v); ?>
                        		<p>
                                <label ><?php _e('Title:','magazon'); ?></label>
                                ...
                                <?php if($current_support['hook']): ?>
                                 <p><label ><?php _e('Apply to hook','magazon'); ?></label>
                                    <select name="<?php echo $name."[$i][hook]" ?>" class="ui-hook">
                                      <option value=""><?php _e('Select','magazon'); ?></option>
                                        <?php 
                                        foreach($hooks as $h): 
                                         $selected='';
                                         if($h==$v['hook']){
                                ...
                                <div class="STpanel-box-upload ui-img-w"><label >Image:</label>
                                    ...
                                    <input class="bp-upload-button button-secondary" type="button" value="<?php echo __('Select Image','magazon'); ?>" />
                                    <a href="#" class="remove_image button-secondary"><?php echo __('Remove','magazon'); ?></a>
                                    <div class="clear"></div>
                                </div>
                                <?php endif; ?>
                                ...
                                <p><label ><input type="checkbox" name="<?php echo $name."[$i][autop]" ?>" <?php echo ($v['autop']==1) ? 'checked="checked"' : ''; ?> class="ui-autop" value="1" />&nbsp;<?php _e('Automatically add paragraphs','magazon'); ?></label></p>
                        	    <div class="widget-description"><?php  _e('Arbitrary text or HTML','magazon'); ?></div>
                                <?php endif; ?>
                        	...
                        	<div class="widget-control-actions">
                        		<div class="alignleft">
                            		...
                            		<a href="#close" class="close"><?php _e('Close','magazon'); ?></a>
                        		</div>
                        		<br class="clear" />
                        	</div>
                ...
                <div class="ui-temp-code" style="display: none;">
                        ...
                        <div title="Click to toggle" class="ui-handlediv"><br></div>
                        <a href="#" class="remove stwrmt button-secondary"><?php _e('Remove','magazon') ?></a>
                        <h3 class="st-hndle"><?php _e('Title:','magazon'); ?><span></span>
                    	...
                    	<div class="inside">
                        	...
                        	<div class="widget-content">
                        		<p><label ><?php _e('Title:','magazon'); ?></label>
                                ...
                                <?php if($current_support['hook']): ?>
                                 ...
                                 <?php endif; ?>
                                  ...
                                  <?php if($current_support['image']): ?>
                                <div class="STpanel-box-upload ui-img-w"><label ><?php _e('Image:','magazon'); ?></label>
                                 ...
                                 <?php if($current_support['url']): ?>
                                 <p><label ><?php _e('url:','magazon'); ?></label>
                                ...
                                <p><label ><input type="checkbox" class="ui-autop" value="1" />&nbsp;<?php _e('Automatically add paragraphs','magazon'); ?></label></p>
                               	<div class="widget-description"><?php _e('Arbitrary text or HTML','magazon'); ?></div>
                                 <?php endif; ?>
